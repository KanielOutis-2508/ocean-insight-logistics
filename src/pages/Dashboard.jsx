...
import { Truck, Route, Package, TrendingUp, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ color: colors.text, fontSize: '1.5rem', fontWeight: 700, marginBottom: '0.25rem' }}>
          Fleet Overview
        </h1>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
          <h2 style={{ color: colors.text, fontSize: '1.1rem', fontWeight: 600 }}>Live Fleet Map</h2>
          <div style={{ display: 'flex', gap: '1rem', fontSize: '0.75rem', color: colors.textLight }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: `1px solid ${colors.border}` }}>
          <h2 style={{ color: colors.text, fontSize: '1rem', fontWeight: 600 }}>Recent Trips</h2>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem', minWidth: '650px' }}>
              ...
              <tr style={{ borderBottom: `1px solid ${colors.border}` }}>
                {['Trip ID', 'Driver', 'Route', 'Cargo', 'Revenue', 'Status'].map(h => (
                  <th key={h} style={{
                    ...
                    color: colors.tableHeader, fontWeight: 600, fontSize: '0.75rem',
            ...
            <tbody>
              {recentTrips.map((trip, i) => (
                <tr key={trip.id} style={{ borderBottom: i < recentTrips.length - 1 ? `1px solid ${colors.border}` : 'none' }}>
                  <td style={{ padding: '1rem 1.5rem', color: '#60a5fa', fontWeight: 600 }}>{trip.id}</td>
                  <td style={{ padding: '1rem 1.5rem', color: colors.text }}>{trip.driver}</td>
                  <td style={{ padding: '1rem 1.5rem', color: colors.textLight, whiteSpace: 'nowrap' }}>
                    {trip.origin} → {trip.destination}
                  </td>
                  <td style={{ padding: '1rem 1.5rem', color: colors.textLight }}>
